...
@testset "System <-> PowerFlowData round trip" begin
    ...
    data_original = PowerFlowData(NLSolveACPowerFlow(), sys_original)

    sys_modified = deepcopy(sys_original)
    modify_rts_system!(sys_modified)
    data_modified = PowerFlowData(NLSolveACPowerFlow(), sys_original)
    ...
    data_null_updated = PowerFlowData(NLSolveACPowerFlow(), sys_null_updated)
    ...
    @test IS.compare_values(
        powerflow_match_fn,
        PowerFlowData(NLSolveACPowerFlow(), sys_modified),
        data_modified,
    )
    ...
    PF.update_system!(sys_mod_redist, PowerFlowData(NLSolveACPowerFlow(), sys_mod_redist))
