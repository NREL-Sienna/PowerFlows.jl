...
function Write_Sienna2PSSE(sys_location::System, scenario_name::String, year::Int64;
    ...
    setpoint_ts::Union{Nothing,Dates.DateTime} = nothing, results_dir::Union{Nothing,String} = nothing, v33 = true)
    ...
    if (setpoint)
        if (setpoint_ts === nothing || results_dir === nothing)
            error("The export timestamp and/or sim store results directory must be specifed.")
        end 
    end
    ...
    base_case ? push!(raw_file_metadata, "Base_Case_Export" => true) : push!(raw_file_metadata, "Base_Case_Export" => false)
    ...
    if (v33)
        push!(raw_file, "0,   $(get_base_power(sys)), 33, 0, 1, 60.00     /  PSS(R)E 33 RAW created by Sienna2PF.jl  $(dt_now)")
    else
        push!(raw_file, "0,   $(get_base_power(sys)), 34, 0, 1, 60.00 /  PSS(R)E 34 RAW created by Sienna2PF.jl  $(dt_now)") 
    end
    ...
    if ~(v33)
        ...
        push!(raw_file, "NEWTON, ITMXN=20, ACCN=1.0, TOLN=0.1, VCTOLQ=0.1, VCTOLV=0.00001, DVLIM=0.99, NDVFCT=0.99")
        push!(raw_file, "ADJUST, ADJTHR=0.005, ACCTAP=1.0, TAPLIM=0.05, SWVBND=100.0, MXTPSS=99, MXSWIM=10")
        ...
        push!(raw_file,"@!   I,'NAME        ', BASKV, IDE,AREA,ZONE,OWNER, VM,        VA,    NVHI,   NVLO,   EVHI,   EVLO")
    ...
    if (v33)
        ...
        push!(raw_file,"0 / END OF BUS DATA, BEGIN LOAD DATA")
        push!(raw_file,"@!   I,'ID',STAT,AREA,ZONE,      PL,        QL,        IP,        IQ,        YP,        YQ, OWNER,SCALE,INTRPT,  DGENP,     DGENQ, DGENF")
    end
    ...
    if(v33)
        ...
        push!(raw_file,"0 / END OF FIXED SHUNT DATA, BEGIN GENERATOR DATA")
        push!(raw_file,"@!   I,'ID',      PG,        QG,        QT,        QB,     VS,    IREG,     MBASE,     ZR,         ZX,         RT,         XT,     GTAP,STAT, RMPCT,      PT,        PB,    O1,    F1,  O2,    F2,  O3,    F3,  O4,    F4,WMOD,  WPF")
    end
    # Generators
    ...
    if (v33)
        ...
        push!(raw_file,"0 / END OF GENERATOR DATA, BEGIN BRANCH DATA")
        push!(raw_file,"@!   I,     J,'CKT',     R,          X,         B,                    'N A M E'                 ,   RATE1,   RATE2,   RATE3,    GI,       BI,       GJ,       BJ,STAT,MET,  LEN,  O1,  F1,    O2,  F2,    O3,  F3,    O4,  F4")
    end

    # Branch
    psy_branches = collect(PSY.get_components((x -> PSY.get_from_bus(x) ∈ psy_buses || PSY.get_to_bus(x) ∈ psy_buses), PSY.ACBranch,sys));
    ...
    for branch in psy_branches
        if typeof(branch) == Line
            ...
            # V34
            name = filter(x -> !isspace(x), replace(first(PSY.get_name(branch),40), r"," => ""))
    ...
    if (v33)
        ...
        push!(raw_file, "@! I, J, K, CKT, CW, CZ, CM, MAG1, MAG2, NMETER, 'NAME', STAT, O1, F1, O2, F2, O3, F3, O4, F4, VECGRP")
        push!(raw_file, "@! R1-2, X1-2, SBASE1-2")
        push!(raw_file, "@! WINDV1, NOMV1, ANG1, RATE11, RATE12, RATE13, COD1, CONT1, RMA1, RMI1, VMA1, VMI1")
    ...
    if (v33)
        ...
        push!(raw_file,"0 /End of Impedance correction table data, Begin Multi-terminal dc line data")
        push!(raw_file,"0 /End of Multi-terminal dc line data, Begin Multi-section line data")
        ...
        push!(raw_file,"0 / END OF IMPEDANCE CORRECTION DATA, BEGIN MULTI-TERMINAL DC DATA")
