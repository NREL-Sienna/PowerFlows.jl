...
function compare_begin_to_final(frame_before :: DataFrame, frame_after :: DataFrame)
    ...
    del_names = DataFrame(["Δ"*name => zeros(nrow(frame_before)) for name in orig_names[2:end]])
    frame_combined = hcat(
        rename(frame_before, [n => n*"_before" for n in names(frame_before)[2:end]]),
        select(rename(frame_after, [n => n*"_after" for n in names(frame_after)]), Not(1))
    )
    for name in orig_names[2:end]
        del_names[!, Symbol("Δ"*name)] = frame_combined[!, Symbol(name*"_before")] .- frame_combined[!, Symbol(name*"_after")]
    end
