...
function compare_begin_to_final(frame_before::DataFrame, frame_after::DataFrame)
    ...
    del_names =
        DataFrame([
            "Δ" * name => zeros(DataFrames.nrow(frame_before)) for name in orig_names[2:end]
        ])
    frame_combined = hcat(
        DataFrames.rename(
            frame_before,
            [n => n * "_before" for n in names(frame_before)[2:end]],
        ),
        DataFrames.select(
            DataFrames.rename(frame_after, [n => n * "_after" for n in names(frame_after)]),
            DataFrames.Not(1),
        ),
    )
