        ...
        elseif get_lcc_count(data) > 0 && converged
            ...
            for (i, (bus_indices, self_admittances)) in
                ...
                data.lcc.arc_activepower_flow_from_to[i, time_step] =
                    real(S_rectifier)
                data.lcc.arc_reactivepower_flow_from_to[i, time_step] =
                    imag(S_rectifier)
                data.lcc.arc_activepower_flow_to_from[i, time_step] =
                    real(S_inverter)
                data.lcc.arc_reactivepower_flow_to_from[i, time_step] =
                    imag(S_inverter)
            end
        end
    end
