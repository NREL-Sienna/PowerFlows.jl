...
##############################################################################
# Matrix Methods #############################################################
# All threaded operations are gated on THREADED_MUL_MIN_DIM (defined in
# definitions.jl) and Threads.nthreads() > 1 to avoid overhead on small systems.

# ── Threaded transpose sparse-matrix × dense vector/matrix ─────────────────
# In CSC format, computing Aᵀx is naturally data-parallel: each output element
# y[i] depends only on column i of A, so threads never contend on writes.
...
# Generic fallback for dense or other matrix types — delegates to BLAS.
function threaded_mul!(
    y::AbstractVecOrMat,
