...
function PowerFlowData(
    ...
    n_timesteps = get_time_steps(pf)
    timestep_names = get_time_step_names(pf)
    if n_timesteps != 0
        if length(timestep_names) == 0
            timestep_names = [string(i) for i in 1:n_timesteps]
        elseif length(timestep_names) != n_timesteps
            error("time_step_names field must have same length as time_steps")
        end
    end
...
function make_and_initialize_powerflow_data(
    ...
    initialize_powerflow_data!(data, pf, sys; correct_bustypes = get_correct_bustypes(pf))
    return data
end
...
- [`pf::vPTDFDCPowerFlow`](@ref vPTDFDCPowerFlow):
        Run a virtual PTDF powerflow: internally, store the virtual PTDF matrix as
