# in order to use this, add MUMPS.jl, MPI.jl, and MPIPreferences.jl
# to your project dependencies. They aren't there by default because MUMPS.jl
# doesn't compile on Windows. [Perhaps this should be an extension.]
...
function symbolic_factor!(mumpsSolver::MUMPSHessianSolver, H::SparseMatrixCSC)
    ...
    if mumpsSolver.mumps.infog[1] < 0
        throw(
            ErrorException(
                "MUMPS symbolic factorization failed with " *
...
function solve!(mumpsSolver::MUMPSHessianSolver, b::Vector{Float64})
    ...
    if mumpsSolver.mumps.infog[1] < 0
        throw(
            ErrorException(
                "MUMPS solve failed with error code " *
