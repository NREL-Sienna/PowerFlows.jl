...
function newton_polar_powerflow!(
    ...
    it = 0
    x_converged, f_converged = assess_convergence(initial_x, cache.xold, value(obj), NaN, ftol)
    ...
    while !stopped && !converged && it < iterations
        ...
        copyto!(cache.x, x_ls)
        x_converged, f_converged = assess_convergence(cache.x, cache.xold, value(obj), xtol, ftol)
        stopped = any(isnan, cache.x) || any(isnan, value(obj)) ? true : false
